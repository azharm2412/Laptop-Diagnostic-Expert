...
def _validate_cf(value: float, name: str = "CF") -> None:
    ...
    Raises:
        ...
        ValueError: Jika value di luar rentang [0.0, 1.0].
    """
    ...
    if not (0.0 <= value <= 1.0):
        raise ValueError(
            f"{name} harus dalam rentang 0.0 - 1.0, "
            f"diterima: {value}"
        )
...
class CertaintyFactor:
    ...
    def combine_cf(cf1: float, cf2: float) -> float:
        """
        Kombinasikan dua nilai CF menggunakan formula:
        CF_combine(CF1, CF2) = CF1 + CF2 × (1 - CF1)

        Args:
            cf1: Nilai CF pertama (0.0 - 1.0).
            cf2: Nilai CF kedua (0.0 - 1.0).

        Returns:
            float: Nilai CF kombinasi (0.0 - 1.0).

        Raises:
            TypeError: Jika cf1 atau cf2 bukan numerik.
            ValueError: Jika cf1 atau cf2 di luar rentang [0.0, 1.0].
    ...
    def calculate_cf_kombinasi(cf_pakar: float, cf_user_list: list[float]) -> float:
        ...
        dimana CF_user_kombinasi dihitung dengan combine_cf_list()
        atas seluruh jawaban CF User yang terpenuhi.
    ...
    def combine_multi_rules(cf_finals: list[float]) -> float:
        ...
        kerusakan yang sama (misal: K08 → R8, R11, R12).
