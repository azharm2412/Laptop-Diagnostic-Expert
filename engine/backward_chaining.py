    ...
    for hypothesis in hypotheses_to_eval:
        ...
        for rule in rules:
            ...
            if rule_passed and cf_users_for_rule:
                cf_user_combined = CertaintyFactor.combine_cf_list(cf_users_for_rule)
                ...
                cf_steps = []
                if len(cf_users_for_rule) >= 2:
                    temp = cf_users_for_rule[0]
                    for i in range(1, len(cf_users_for_rule)):
                        new_temp = CertaintyFactor.combine_cf(temp, cf_users_for_rule[i])
                        cf_steps.append({
                            "operation": "combine",
                            "cf1": temp,
                            "cf2": cf_users_for_rule[i],
                            "result": new_temp,
                        })
                        temp = new_temp
                cf_steps.append({
...
class BackwardChainingEngine:
    ...
    def get_progress(self) -> tuple[int, int]:
        ...
        """
        return (self.current_hyp_idx + 1, len(self.hypotheses))
    ...
    def _finalize_current_rule(self) -> None:
        ...
        cf_steps = []
        if len(self._current_cf_users) >= 2:
            temp = self._current_cf_users[0]
            for i in range(1, len(self._current_cf_users)):
                new_temp = CertaintyFactor.combine_cf(temp, self._current_cf_users[i])
                cf_steps.append({
                    "operation": "combine",
                    "cf1": temp,
                    "cf2": self._current_cf_users[i],
                    "result": new_temp,
                })
                temp = new_temp
        cf_steps.append({
            "operation": "final",
            "cf_rule": rule["cf_rule"],
            "cf_user_combined": CertaintyFactor.combine_cf_list(self._current_cf_users),
            "cf_final": cf_final,
        })
