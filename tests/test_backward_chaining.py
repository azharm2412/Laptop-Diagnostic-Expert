...
class TestBackwardChainFunction:
    ...
    def test_single_goal_k01_terbukti(self):
        ...
        assert result["ranking"][0]["cf_final"] == pytest.approx(0.90, abs=1e-2)
    ...
    def test_cf_user_negatif_raise_error(self):
        """CF User negatif harus raise ValueError."""
        # Act & Assert
        with pytest.raises(ValueError, match="harus dalam rentang"):
            backward_chain(goal="K01", user_answers={"G01": -0.5})
    ...
    def test_multi_rule_k08(self):
        """
        K08 memiliki 3 rule (R8, R11, R12).
        Jika R8 gagal tapi R11+R12 terbukti → K08 TERBUKTI via combine.
        """
        # Arrange — R8 gagal (G19=0.0), R11 terbukti (G24), R12 terbukti (G25)
        answers = {
            "G19": 0.0,   # R8 akan short-circuit
            "G24": 0.8,   # R11 terbukti
            "G25": 0.6,   # R12 terbukti
        }
...
class TestEngineHypothesisProven:
    ...
    def test_k01_terbukti_semua_positif(self):
        ...
        assert k01["cf_final"] == pytest.approx(0.90, abs=1e-2)
