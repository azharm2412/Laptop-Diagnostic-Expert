...
class TestCombineCF:
    ...
    def test_input_negatif_raise_error(self):
        """CF negatif harus ditolak."""
        with pytest.raises(ValueError):
            CertaintyFactor.combine_cf(-0.1, 0.5)
...
class TestCalculateCFKombinasi:
    ...
    def test_simulasi_rule_r1_k01(self):
        ...
        Simulasi R1 → K01 (Adaptor Rusak):
          ...
          CF_user_combined = 1.0
          CF_final = 0.90 × 1.0 = 0.90
        """
        # Arrange
        cf_pakar = 0.90
        cf_users = [0.8, 0.6, 1.0]
        ...
        # Assert
        assert result == pytest.approx(0.90, abs=1e-4)
    ...
    def test_simulasi_rule_r4_k04(self):
        ...
        Simulasi R4 → K04 (Overheating):
          ...
          CF_user_combined = combine(combine(0.8,0.6),0.8) = combine(0.92,0.8) = 0.984
          CF_final = 0.88 × 0.984 = 0.8659
        """
        # Arrange
        cf_pakar = 0.88
        cf_users = [0.8, 0.6, 0.8]
        ...
        # Assert
        expected = 0.88 * (0.8 + 0.6 * 0.2 + 0.8 * (1 - 0.92))  # 0.88 × 0.984
    ...
    def test_validasi_cf_pakar_negatif(self):
        """CF pakar negatif harus ditolak."""
        with pytest.raises(ValueError):
            CertaintyFactor.calculate_cf_kombinasi(-0.5, [0.8])

    def test_validasi_cf_user_negatif(self):
        """CF user negatif di dalam list harus ditolak."""
        with pytest.raises(ValueError):
            CertaintyFactor.calculate_cf_kombinasi(0.80, [0.8, -0.2])


# ============================================================
# TEST: combine_multi_rules() — Multi-Rule (K08: R8, R11, R12)
# ============================================================

class TestCombineMultiRules:
    """Test suite untuk multi-rule CF combine."""

    def test_dua_rules_k08(self):
        """
        Simulasi K08 (Motherboard) hanya R11+R12 terbukti:
        ...
        # Act
        result = CertaintyFactor.combine_multi_rules([cf_r11, cf_r12])
